...
def index() -> Any:
    ...
    try:
        files = [
            ...
            if f.startswith("ts_") and f.endswith(".csv")
        ]
    except FileNotFoundError:
        current_app.logger.error(
            f"Configured DATA_FOLDER does not exist: {data_folder}"
        )
        files = []
    except Exception as e:
        current_app.logger.error(
            f"Error listing files in data folder {data_folder}: {e}"
        )
        files = []
    ...
    metric_display_names = sorted(
        [name[3:] for name in metric_filenames]
    )  # Remove 'ts_' prefix
    ...
    current_app.logger.info(
        "Starting Change Z-score aggregation for dashboard (ts_ files only)..."
    )
    ...
    for metric_filename in metric_filenames:
        ...
        try:
            ...
            df, fund_cols, benchmark_col, _sec_df, _sec_fund_cols, _sec_bench_col = (
            ...
            if df is None:
                current_app.logger.warning(
                    f"Warning: Failed to load data for {filename}. Skipping."
                )
            ...
            if not benchmark_col and not fund_cols:
                current_app.logger.warning(
                    f"Warning: No benchmark or fund columns identified in {filename}. Skipping."
                )
                continue
            ...
            if not latest_metrics.empty:
                ...
                if not columns_to_check:
                    current_app.logger.warning(
                        f"Warning: No columns to check for Z-scores in {filename} despite loading data."
                    )
                    continue

                current_app.logger.info(
                    f"Checking for Z-scores for columns: {columns_to_check} in metric {display_name}"
                )
                ...
                for original_col_name in columns_to_check:
                    ...
                    if z_score_col_name in latest_metrics.columns:
                        ...
                        # Extract and rename
                        metric_z_scores = latest_metrics[[z_score_col_name]].rename(
                            columns={z_score_col_name: summary_col_name}
                        )
                        ...
                        current_app.logger.info(
                            f"  -> Z-score column '{z_score_col_name}' not found."
                        )

                if not found_z_for_metric:
                    current_app.logger.warning(
                        f"Warning: No Z-score columns found for any checked column in metric {display_name} (from {filename})."
                    )

            else:
                current_app.logger.warning(
                    f"Warning: Could not calculate latest_metrics for {filename}. Skipping Z-score extraction."
                )
            ...
            current_app.logger.error(
                f"Value Error processing {metric_filename}: {ve}"
            )  # Log with filename
        except Exception as e:
            current_app.logger.error(
                f"Error processing {metric_filename} during dashboard aggregation: {e}"
            )  # Log with filename
            traceback.print_exc()
    ...
    if all_z_scores_list:
        ...
        if processed_summary_columns:
            ...
            cols_available_in_summary = [
                col for col in processed_summary_columns if col in summary_df.columns
            ]
        ...
        current_app.logger.info(
            "No Change Z-scores could be extracted for the summary."
        )
    ...
    return render_template(
        ...
        summary_metrics=processed_summary_columns,
    )  # Pass the NEW list of combined column names
