...
def _melt_data(df, id_vars, fund_code):
    ...
    current_app.logger.debug(
        f"[_melt_data] Processing for fund '{fund_code}'. Input shape: {df.shape}, Columns: {df.columns.tolist()}"
    )
    ...
    try:
        if "Funds" not in df.columns:
            current_app.logger.error(
                f"[_melt_data] Critical Error: 'Funds' column not found in input DataFrame for fund '{fund_code}'. Columns: {df.columns.tolist()}"
            )
            return None
        ...
        def check_fund_match(fund_entry, target_fund):
            ...
            if (
                isinstance(fund_entry, str)
                and fund_entry.startswith("[")
                and fund_entry.endswith("]")
            ):
                # Strip brackets
        ...
        current_app.logger.error(
            f"[_melt_data] Error processing 'Funds' column during filtering for '{fund_code}': {e}",
            exc_info=True,
        )
        return None

    current_app.logger.debug(
        f"[_melt_data] Shape after filtering for '{fund_code}': {df_filtered.shape}"
    )
    if df_filtered.empty:
        current_app.logger.warning(
            f"[_melt_data] No data found for fund_code '{fund_code}' after filtering."
        )
    ...
    current_app.logger.debug(
        f"[_melt_data] Identified non-date columns (excluding ids): {non_date_cols}"
    )

    if not date_cols:
        current_app.logger.error(
            "[_melt_data] Could not identify any date columns for melting."
        )
        return None
    ...
    try:
        current_app.logger.debug(
            f"[_melt_data] Attempting melt with id_vars={id_vars}, value_vars={date_cols[:5]}..."
        )
        ...
        current_app.logger.debug(
            f"[_melt_data] Melt successful. Shape after melt: {melted_df.shape}"
        )

        melted_df["Date"] = pd.to_datetime(melted_df["Date"], errors="coerce")
        melted_df = melted_df.dropna(subset=["Date"])
        current_app.logger.debug(
            f"[_melt_data] Date conversion done. Shape after dropping NaT dates: {melted_df.shape}"
        )
        return melted_df
    except Exception as melt_err:
        current_app.logger.error(
            f"[_melt_data] Error melting dataframe: {melt_err}", exc_info=True
        )
        return None


def _calculate_contributions(
    metric_name, fund_code, start_date_str, end_date_str, data_source, top_n=10
):
    ...
    current_app.logger.info(
        f"Calculation Params: Baseline='{baseline_date.strftime('%Y-%m-%d')}', TopN='{top_n}'"
    )
    ...
    if weights_long is None:
        ...
        raise ValueError(
            f"Could not process weights data for fund '{fund_code}' from {weights_filename}."
        )
    if weights_long.empty:
        raise ValueError(
            f"No weight data found for fund '{fund_code}' in {weights_filename}. Calculation cannot proceed."
        )
    ...
    if metric_df_raw is None:
        raise FileNotFoundError(
            f"Metric data file not found or failed to load: {metric_filename}. Ensure the file exists and is readable."
        )
    metric_long = _melt_data(metric_df_raw, metric_id_vars, fund_code)
    if metric_long is None:
        raise ValueError(
            f"Could not process metric data for fund '{fund_code}' from {metric_filename}."
        )
    if metric_long.empty:
        raise ValueError(
            f"No metric data found for fund '{fund_code}' in {metric_filename}. Calculation cannot proceed."
        )

    metric_long.rename(columns={"Value": "MetricValue"}, inplace=True)
    metric_long["MetricValue"] = pd.to_numeric(
        metric_long["MetricValue"], errors="coerce"
    )
    ...
    merged_data = pd.merge(
        metric_long,
        weights_long[["ISIN", "Date", "Weight"]],
        on=["ISIN", "Date"],
        how="inner",
    )
    if merged_data.empty:
        raise ValueError(
            "No matching data found between weights and metric values for the specified fund and dates."
        )
    current_app.logger.info(f"Merged data has {len(merged_data)} records.")

    merged_data["DailyContribution"] = (
        merged_data["Weight"] * merged_data["MetricValue"]
    )
    merged_data = merged_data.dropna(subset=["DailyContribution"])
    if merged_data.empty:
        raise ValueError(
            "Could not calculate daily contributions (check for NaNs in weights or metric values)."
        )
    current_app.logger.info("Daily contributions calculated.")

    baseline_data = merged_data[merged_data["Date"] == baseline_date].copy()
    baseline_contribs = baseline_data.drop_duplicates(subset=["ISIN"], keep="last")[
        ["ISIN", "DailyContribution"]
    ]
    baseline_contribs = baseline_contribs.set_index("ISIN")["DailyContribution"]
    current_app.logger.info(
        f"Found {len(baseline_contribs)} baseline contributions for date {baseline_date.strftime('%Y-%m-%d')}."
    )
    ...
    if period_data.empty:
        raise ValueError(
            f"No data found within the selected period {start_date_str} to {end_date_str}."
        )

    average_contribs = period_data.groupby("ISIN")["DailyContribution"].mean()
    current_app.logger.info(
        f"Calculated average contributions for {len(average_contribs)} securities over the period."
    )

    results_df = pd.DataFrame(average_contribs).rename(
        columns={"DailyContribution": "AverageContribution"}
    )
    results_df["BaselineContribution"] = results_df.index.map(baseline_contribs)

    results_df = results_df.dropna(
        subset=["BaselineContribution", "AverageContribution"]
    )
    if results_df.empty:
        raise ValueError(
            "Could not compare average to baseline (possibly missing baseline data for all relevant securities)."
        )

    results_df["ContributionDifference"] = (
        results_df["AverageContribution"] - results_df["BaselineContribution"]
    )
    results_df = results_df[results_df["ContributionDifference"] != 0]
    if results_df.empty:
        current_app.logger.warning(
            "No securities found with non-zero contribution difference."
        )
        ...
        current_app.logger.info(
            f"Calculated contribution difference for {len(results_df)} securities."
        )
        ...
        try:
            ...
            if reference_df is not None and not reference_df.empty:
                ...
                if (
                    "ISIN" in reference_df.columns
                    and "Security Name" in reference_df.columns
                ):
                    # Prepare the lookup dictionary {ISIN: Security Name}
                    sec_names_lookup = (
                        reference_df.drop_duplicates(subset=["ISIN"])
                        .set_index("ISIN")["Security Name"]
                        .to_dict()
                    )
                    ...
                    results_df["Security Name"] = results_df.index.map(
                        lambda isin: sec_names_lookup.get(isin, isin)
                    )
                    current_app.logger.info(
                        f"Successfully merged security names from reference.csv for {results_df['Security Name'].notna().sum()} / {len(results_df)} securities."
                    )
                else:
                    # --- CORRECTED: Log message for reference.csv ---
                    current_app.logger.warning(
                        "'ISIN' or 'Security Name' column not found in reference.csv. Cannot merge names."
                    )
                ...
                # --- CORRECTED: Log message for reference.csv ---
                current_app.logger.warning(
                    "reference.csv empty or could not be loaded. Using ISIN as name."
                )
                results_df["Security Name"] = results_df.index  # Default to ISIN
            ...
            # --- CORRECTED: Log message for reference.csv ---
            current_app.logger.error(
                f"Error loading or merging reference.csv: {e}. Using ISIN as name.",
                exc_info=True,
            )
            results_df["Security Name"] = (
                results_df.index
            )  # Default to ISIN in case of other errors
        ...
        top_contributors_list = make_json_safe(
            top_contributors.reset_index()[output_columns].to_dict(orient="records")
        )
        top_detractors_list = make_json_safe(
            top_detractors.reset_index()[output_columns].to_dict(orient="records")
        )
    ...
    current_app.logger.info(
        f"Successfully calculated contributions. Found {len(top_contributors_list)} contributors, {len(top_detractors_list)} detractors."
    )
    return results_json
...
def inspect_metric_contribution(metric_name):
    ...
    if not data:
        current_app.logger.error(
            f"Debug - Invalid request: No JSON body received. Content-Type: {request.content_type}"
        )
    ...
    if not all([start_date_str, end_date_str, fund_code]):
        return (
            jsonify(
                {
                    "error": "Missing required parameters: 'start_date', 'end_date', and 'fund_code' are required."
                }
            ),
            400,
        )
    ...
    try:
        ...
        current_app.logger.error(
            f"Inspect API Error: File not found - {e}", exc_info=True
        )
        ...
        current_app.logger.error(
            f"Inspect API Error: Missing column - {e}", exc_info=True
        )
...
def inspect_results_page():
    ...
    top_n = request.args.get(
        "top_n", 10, type=int
    )  # Allow overriding top_n via query param if needed

    if not all([metric_name, fund_code, start_date_str, end_date_str]):
        return (
            render_template(
                "error_page.html",
                error_message="Missing parameters for inspect results (metric_name, fund_code, start_date, end_date required).",
            ),
            400,
        )

    current_app.logger.info(
        f"Rendering Inspect results page for {metric_name}, {fund_code} [{start_date_str} - {end_date_str}], Source: {data_source}"
    )
    ...
    try:
        ...
        current_app.logger.error(
            f"Unexpected error calculating inspect results for page: {e}", exc_info=True
        )
        error_message = (
            f"An unexpected server error occurred while calculating results."
        )
        contribution_data = {}  # Ensure empty dict on error
