...
def load_watchlist(data_folder_path: str):
    ...
    try:
        if os.path.exists(watchlist_path) and os.path.getsize(watchlist_path) > 0:
            df = pd.read_csv(watchlist_path, parse_dates=['DateAdded', 'LastChecked', 'ClearedDate'])
...
def clear_watchlist_entry(data_folder_path: str, isin: str, cleared_by: str, clear_reason: str):
    """
...
def check_watchlist_entry(isin):
    ...
    return redirect(url_for('security.security_details', metric_name='Duration', security_id=isin)) 