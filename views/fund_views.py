...
@fund_bp.route(
    "/duration_details/<fund_code>"
)  # Corresponds to /fund/duration_details/...
def fund_duration_details(fund_code):
    ...
    current_app.logger.info(
        f"--- Requesting Duration Details for Fund: {fund_code} --- File: {data_filepath}"
    )

    if not os.path.exists(data_filepath):
        current_app.logger.error(
            f"Error: Duration file '{duration_filename}' not found."
        )
    ...
    try:
        ...
        if id_col_name not in all_cols:
            ...
            return (
                f"Error: Required ID column '{id_col_name}' not found in '{duration_filename}'.",
                500,
            )
        ...
        current_app.logger.info(
            f"Dynamically identified Date Cols (first 5): {date_cols[:5]}..."
        )

        if not date_cols or len(date_cols) < 2:
            current_app.logger.error(
                "Error: Not enough date columns found in duration file to calculate change."
            )
            return (
                f"Error: Insufficient date columns in '{duration_filename}' to calculate change.",
                500,
            )
        ...
        if funds_col not in static_cols:
            ...
            return (
                f"Error: Required column '{funds_col}' for fund filtering not found.",
                500,
            )
        ...
        try:
            ...
            date_cols = sorted(
                date_cols, key=lambda d: pd.to_datetime(d, format="%Y-%m-%d")
            )
            current_app.logger.info(
                f"Identified and sorted date columns (YYYY-MM-DD): {date_cols[-5:]} (last 5 shown)"
            )
        except ValueError:
            current_app.logger.warning(
                "Warning: Could not parse all date columns using YYYY-MM-DD format. Using original order."
            )
        ...
        if len(date_cols) < 2:  # Double check after potential parsing failure
            return (
                f"Error: Insufficient valid date columns in '{duration_filename}' to calculate change after sorting attempt.",
                500,
            )
        last_date_col = date_cols[-1]
        second_last_date_col = date_cols[-2]
        current_app.logger.info(
            f"Using dates for change calculation: {second_last_date_col} and {last_date_col}"
        )
        ...
        df[second_last_date_col] = pd.to_numeric(
            df[second_last_date_col], errors="coerce"
        )
        ...
        if not os.path.exists(weights_filepath):
            current_app.logger.warning(
                f"Warning: Weight file '{weights_filename}' not found. Skipping duration contribution calculation."
            )
        else:
            try:
                ...
                try:
                    ...
                    second_last_date_dt = pd.to_datetime(
                        second_last_date_col, format="%Y-%m-%d"
                    )
                    last_date_col_weights_fmt = last_date_dt.strftime("%d/%m/%Y")
                    second_last_date_col_weights_fmt = second_last_date_dt.strftime(
                        "%d/%m/%Y"
                    )
                    current_app.logger.info(
                        f"Looking for weight columns: {last_date_col_weights_fmt}, {second_last_date_col_weights_fmt}"
                    )
                except ValueError:
                    current_app.logger.error(
                        f"Error: Could not convert duration dates ({last_date_col}, {second_last_date_col}) to datetime objects for weight lookup. Skipping contribution."
                    )
                    last_date_col_weights_fmt = (
                        None  # Ensure it skips if conversion fails
                    )

                # Check using the formatted date strings
                required_weight_cols = [weight_id_col, weight_fund_col]
                if last_date_col_weights_fmt:
                    required_weight_cols.extend(
                        [last_date_col_weights_fmt, second_last_date_col_weights_fmt]
                    )
                else:
                    # Skip if dates couldn't be formatted
                    current_app.logger.warning(
                        f"Skipping weight check due to date format conversion error."
                    )
                    all_cols_exist = False

                all_cols_exist = all(
                    col in weights_df.columns for col in required_weight_cols
                )
                ...
                if not all_cols_exist:
                    ...
                    current_app.logger.warning(
                        f"Warning: Weight file '{weights_filename}' is missing required columns (needed: {required_weight_cols}, missing: {missing_cols}). Skipping contribution calculation."
                    )
                else:
                    ...
                    fund_weights_df = weights_df[
                        weights_df[weight_fund_col] == fund_code
                    ].copy()

                    if fund_weights_df.empty:
                        current_app.logger.warning(
                            f"Warning: No weights found for fund '{fund_code}' in {weights_filename}. Contribution will be zero."
                        )
                        # Create empty df with correct columns to avoid merge errors later if we still want zero cols
                        weights_to_merge = pd.DataFrame(
                            columns=[
                                weight_id_col,
                                "Weight Last Date",
                                "Weight Second Last Date",
                            ]
                        )
                        weights_to_merge = weights_to_merge.astype(
                            {
                                weight_id_col: "object",
                                "Weight Last Date": "float64",
                                "Weight Second Last Date": "float64",
                            }
                        )
                    else:
                        # Select and rename relevant weight columns using the CORRECT formatted date strings
                        weights_to_merge = fund_weights_df[
                            [
                                weight_id_col,
                                last_date_col_weights_fmt,
                                second_last_date_col_weights_fmt,
                            ]
                        ].copy()
                        weights_to_merge.rename(
                            columns={
                        ...
                        weights_to_merge["Weight Last Date"] = pd.to_numeric(
                            weights_to_merge["Weight Last Date"], errors="coerce"
                        )
                        weights_to_merge["Weight Second Last Date"] = pd.to_numeric(
                            weights_to_merge["Weight Second Last Date"], errors="coerce"
                        )

                    # Merge weights with filtered duration data
                    current_app.logger.info(
                        f"Merging duration data with weights on '{weight_id_col}'"
                    )
                    filtered_df = pd.merge(
                        filtered_df, weights_to_merge, on=weight_id_col, how="left"
                    )
                    ...
                    filtered_df[contrib_last_col] = (
                        filtered_df[last_date_col] * filtered_df["Weight Last Date"]
                    )
                    filtered_df[contrib_second_last_col] = (
                        filtered_df[second_last_date_col]
                        * filtered_df["Weight Second Last Date"]
                    )
                    filtered_df[contrib_change_col] = (
                        filtered_df[contrib_last_col]
                        - filtered_df[contrib_second_last_col]
                    )
                    ...
                    new_contribution_cols = [
                        contrib_second_last_col,
                        contrib_last_col,
                        contrib_change_col,
                    ]
                    current_app.logger.info(
                        "Duration contribution calculated successfully."
                    )

            except Exception as weight_err:
                current_app.logger.error(
                    f"Error processing weight file '{weights_filename}': {weight_err}. Skipping contribution calculation."
                )
                traceback.print_exc()
        ...
        if filtered_df.empty:
            current_app.logger.info(
                f"No securities found for fund '{fund_code}' in {duration_filename} after initial filtering."
            )
            # Render a template indicating no data found for this fund
            return render_template(
                "fund_duration_details.html",
                fund_code=fund_code,
                securities_data=[],
                column_order=[],
                id_col_name=None,
        ...
        current_app.logger.info(
            f"Found {len(filtered_df)} securities for fund '{fund_code}'. Calculating duration changes..."
        )
        ...
        if change_col_name not in filtered_df.columns:
            filtered_df[change_col_name] = (
                filtered_df[last_date_col] - filtered_df[second_last_date_col]
            )

        # 5. Sort by Duration Change (descending, NaN last)
        filtered_df.sort_values(
            by=change_col_name, ascending=False, na_position="last", inplace=True
        )
        ...
        numeric_cols_in_final = (
            filtered_df[final_col_order].select_dtypes(include=np.number).columns
        )
        filtered_df[numeric_cols_in_final] = filtered_df[numeric_cols_in_final].round(
            4
        )  # Use more precision for contribution?
        ...
        current_app.logger.error(
            f"Error processing duration details for fund {fund_code}: {e}"
        )
        traceback.print_exc()
        return (
            f"An error occurred processing duration details for fund {fund_code}: {e}",
            500,
        )
...
def fund_detail(fund_code):
    ...
    current_app.logger.info(
        f"--- Requesting Detail Page for Fund: {fund_code}, S&P Valid Filter: {filter_sp_valid} ---"
    )
    data_folder = current_app.config[
        "DATA_FOLDER"
    ...
    try:
        ...
        current_app.logger.info(
            f"Found {len(ts_files)} primary ts_ files: {[os.path.basename(f) for f in ts_files]}"
        )

        if not ts_files:
            current_app.logger.warning(
                "No primary ts_*.csv files found in Data folder."
            )
            return render_template(
                "fund_detail_page.html",
                fund_code=fund_code,
                chart_data_json="[]",
                available_metrics=[],
        ...
        for file_path in ts_files:
            ...
            if not match:
                current_app.logger.warning(
                    f"Could not extract metric name from filename: {filename}. Skipping."
                )
                skipped_files += 1
                continue
            ...
            try:
                ...
                load_result = load_and_process_data(
                    ...
                    filter_sp_valid=filter_sp_valid,
                )
                df = load_result[0]
                ...
                if df is None or df.empty:
                    current_app.logger.warning(
                        f"No data loaded or DataFrame empty for {filename}. Skipping."
                    )
                    skipped_files += 1
                    continue
                ...
                current_app.logger.error(
                    f"Error loading primary file {filename}: {e}", exc_info=False
                )
            ...
            if primary_load_error is None and os.path.exists(sp_file_path):
                current_app.logger.info(
                    f"Found corresponding SP file: {sp_filename}. Attempting to load."
                )
                try:
                    # Pass filter_sp_valid to data loader for SP file as well
                    sp_load_result = load_and_process_data(
                        primary_filename=sp_filename,
                        data_folder_path=data_folder,
                        filter_sp_valid=filter_sp_valid,
                    )
                    sp_df = sp_load_result[0]
                    sp_fund_cols = sp_load_result[
                        1
                    ]  # Assuming same structure for fund cols
                    sp_benchmark_col = sp_load_result[
                        2
                    ]  # Get potential SP benchmark col name
                    ...
                    if sp_df is None or sp_df.empty:
                        ...
                        sp_fund_col_name = next(
                            (col for col in sp_fund_cols if col in sp_df.columns), None
                        )
                        if not sp_fund_col_name:
                            current_app.logger.warning(
                                f"Could not find fund data column in SP file {sp_filename}."
                            )
                    ...
                    current_app.logger.error(
                        f"Error loading SP file {sp_filename}: {e}", exc_info=False
                    )
            ...
            if fund_df.empty:
                current_app.logger.info(
                    f"Fund code '{fund_code}' not found in primary data from {filename}. Skipping metric."
                )
                skipped_files += 1  # Processed file, but no data for this fund
                continue  # Skip to next ts_file

            current_app.logger.info(
                f"Fund code '{fund_code}' found in primary data. Preparing chart data for '{metric_name_display}'..."
            )
            ...
            if sp_df is not None:
                ...
                if not sp_fund_df.empty:
                    ...
                    current_app.logger.info(
                        f"Fund code '{fund_code}' *not* found in SP data from {sp_filename}."
                    )
            ...
            fund_col_name = next(
                (col for col in fund_cols if col in fund_df.columns), None
            )
            if fund_col_name:
                # Reindex to ensure consistent length and alignment, fill missing with None
                fund_values = (
                    fund_df[fund_col_name]
                    .reindex(pd.to_datetime(full_date_list))
                    .where(pd.notna, None)
                    .tolist()
                )
                chart_data["datasets"].append(
                    {
                        ...
                        "borderColor": current_app.config["COLOR_PALETTE"][
                            0 % len(current_app.config["COLOR_PALETTE"])
                        ],
                        "tension": 0.1,
                        "pointRadius": 1,
                        "borderWidth": 1.5,
                ...
                current_app.logger.warning(
                    f"Warning: Could not find primary fund data column ({fund_cols}) in {filename} for fund {fund_code}"
                )

            # Add benchmark dataset (from primary data)
            if benchmark_col and benchmark_col in fund_df.columns:
                bench_values = (
                    fund_df[benchmark_col]
                    .reindex(pd.to_datetime(full_date_list))
                    .where(pd.notna, None)
                    .tolist()
                )
                chart_data["datasets"].append(
                    {
                        ...
                        "borderColor": current_app.config["COLOR_PALETTE"][
                            1 % len(current_app.config["COLOR_PALETTE"])
                        ],
                        "tension": 0.1,
                        "pointRadius": 1,
                        "borderDash": [5, 5],
            ...
            if sp_fund_df is not None:
                if sp_fund_col_name:
                    sp_fund_aligned = sp_fund_df[sp_fund_col_name].reindex(
                        pd.to_datetime(full_date_list)
                    )
                    sp_values = sp_fund_aligned.where(
                        pd.notna, None
                    ).tolist()  # Replace NaN with None for JSON
                    chart_data["datasets"].append(
                        {
                            "label": f"{fund_code} {metric_name_display} (SP)",
                            "data": nan_to_none(sp_values),
                            "borderColor": current_app.config["COLOR_PALETTE"][
                                2 % len(current_app.config["COLOR_PALETTE"])
                            ],
                            "tension": 0.1,
                            "pointRadius": 1,
                            "borderDash": [2, 2],
                    ...
                    current_app.logger.info(
                        f"Added SP Fund dataset for metric '{metric_name_display}'."
                    )
                else:
                    current_app.logger.warning(
                        f"SP Fund column ('{sp_fund_cols}') not found in filtered SP data for {sp_filename}, fund {fund_code}."
                    )

                # --- Add SP benchmark dataset (if available) ---
                if sp_benchmark_col and sp_benchmark_col in sp_fund_df.columns:
                    sp_bench_aligned = sp_fund_df[sp_benchmark_col].reindex(
                        pd.to_datetime(full_date_list)
                    )
                    ...
                    chart_data["datasets"].append(
                        {
                            ...
                            "borderColor": current_app.config["COLOR_PALETTE"][
                                3 % len(current_app.config["COLOR_PALETTE"])
                            ],
                            "tension": 0.1,
                            "pointRadius": 1,
                            "borderDash": [2, 2],
                    ...
                    current_app.logger.info(
                        f"Added SP Benchmark dataset ('{sp_benchmark_col}') for metric '{metric_name_display}'."
                    )
                elif sp_benchmark_col:
                    current_app.logger.warning(
                        f"SP Benchmark column ('{sp_benchmark_col}') specified but not found in filtered SP data for {sp_filename}, fund {fund_code}."
                    )
            ...
            if any(d["data"] for d in chart_data["datasets"]):
                ...
                current_app.logger.warning(
                    f"No valid datasets generated for metric '{metric_name_display}' from {filename} (and potentially {sp_filename}). Skipping chart."
                )
        ...
        current_app.logger.info(
            f"Finished processing files for fund {fund_code}. Generated charts for: {available_metrics}. Total Processed: {processed_files}, Skipped/No Data/Errors: {skipped_files}"
        )
        ...
        current_app.logger.error(
            f"Unexpected error in fund_detail for {fund_code}: {e}", exc_info=True
        )
        traceback.print_exc()
