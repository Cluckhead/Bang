...
def _get_threshold_overrides(request_args):
    ...
    if max_str:
        try:
            ...
            current_app.logger.warning(f"Invalid override max value '{max_str}' received.")
    if min_str:
        try:
            override_min = float(min_str)
        except ValueError:
            current_app.logger.warning(f"Invalid override min value '{min_str}' received.")
...
def dashboard(group_name=None):
    ...
    """
    if group_name:
        current_app.logger.info(f"Accessing max/min breach dashboard for group: {group_name}")
        title = f"{group_name.capitalize()} Max/Min Breach Dashboard"
        # Filter the threshold config for the specified group
        threshold_config = { 
            filename: config for filename, config in MAXMIN_THRESHOLDS.items() 
            if config.get('group') == group_name
        }
        if not threshold_config:
            current_app.logger.warning(f"No files found for Max/Min group: {group_name}")
    ...
    try:
        ...
        return render_template('error.html', message=f"Could not generate {title}."), 500
    ...
    try:
        ...
        if max_str is not None:
            try:
                ...
                current_app.logger.warning(f"Invalid max override '{max_str}' for {filename}, using config default.")
        else:
            max_threshold = file_config.get('max', DEFAULT_MAX_THRESHOLD)
        if min_str is not None:
            try:
                ...
                current_app.logger.warning(f"Invalid min override '{min_str}' for {filename}, using config default.")
        else:
            min_threshold = file_config.get('min', DEFAULT_MIN_THRESHOLD)
        ...
        return render_template('error.html', message=f"Data file '{filename}' not found for Max/Min details."), 404
    except Exception as e:
