...
def load_generic_comparison_data(data_folder_path: str, file1: str, file2: str):
    ...
    try:
        ...
        if common_static_cols:
            ...
            static_data = pd.DataFrame({id_level_name: merged_df[id_level_name].unique()})
...
def calculate_generic_comparison_stats(merged_df, static_data, id_col):
    ...
    for sec_id, group in merged_df.groupby(id_col):
        ...
        same_start = (pd.notna(sec_stats['Start_Date_Orig']) and pd.notna(sec_stats['Start_Date_New']) and pd.Timestamp(sec_stats['Start_Date_Orig']) == pd.Timestamp(sec_stats['Start_Date_New']))
        same_end = (pd.notna(sec_stats['End_Date_Orig']) and pd.notna(sec_stats['End_Date_New']) and pd.Timestamp(sec_stats['End_Date_Orig']) == pd.Timestamp(sec_stats['End_Date_New']))
...
def load_fund_codes_from_csv(data_folder: str) -> list:
    ...
    try:
        ...
        if 'Fund Code' in df.columns:
            ...
            logging.getLogger(__name__).warning(f"'Fund Code' column not found in FundList.csv at {fund_list_path}")
            return []
    except Exception as e:
        logging.getLogger(__name__).error(f"Error loading FundList.csv at {fund_list_path}: {e}")
        return [] 