    ...
    try:
        ...
        if exclusions_df is not None and not exclusions_df.empty:
            if id_column in exclusions_df.columns:
                ...
                logger.warning(f"Exclusions DataFrame does not contain expected ID column '{id_column}' or 'SecurityID'. No exclusions will be applied.")
        total_count = 0
        for idx, row in df.iterrows():
            ...
            non_null_values = [v for v in reversed(date_values) if not pd.isna(v) and str(v).strip().lower() not in {'n/a', 'na', '', 'null', 'none'}]
            if len(non_null_values) >= threshold_days:
                last_n = list(reversed(non_null_values[:threshold_days]))
                ref_value = last_n[0]
                all_equal = all(
                    (not pd.isna(val) and not pd.isna(ref_value) and abs(float(val) - float(ref_value)) < FLOAT_TOLERANCE)
                    for val in last_n
                )
                if all_equal:
