...
def replace_headers_with_dates(
    ...
    if date_columns is None:
        ...
        original_placeholder_start_index_B = candidate_start_index + start_index_patternB
        if len(date_columns) == placeholder_count_B:
            ...
            if len(new_columns) == len(current_cols):
                ...
                logger.info(f"Replaced {placeholder_count_B} repeated '{potential_placeholder_base_patternB}' columns with dates.")
            else:
                logger.error(f"Internal error (Pattern B): Column count mismatch after constructing new columns. Keeping original headers.")
        else:
            logger.warning(f"Count mismatch for Pattern B: Found {placeholder_count_B} repeated '{potential_placeholder_base_patternB}' columns, but expected {len(date_columns)} dates. Skipping date replacement.")
        ...
        if len(date_columns) == placeholder_count_A:
            ...
            if len(new_columns) == len(current_cols):
                df.columns = new_columns
                logger.info(f"Replaced {placeholder_count_A} Pattern A columns ('{potential_placeholder_base_patternA}', '{potential_placeholder_base_patternA}.1', ...) with dates.")
            else:
                logger.error(f"Internal error (Pattern A): Column count mismatch after constructing new columns. Keeping original headers.")
        else:
            logger.warning(f"Count mismatch for Pattern A: Found {placeholder_count_A} columns in sequence ('{potential_placeholder_base_patternA}', '{potential_placeholder_base_patternA}.1', ...), but expected {len(date_columns)} dates. Skipping date replacement.")
    ...
    for sec_name, sec_group in grouped_by_sec:
        ...
        if id_cols:
            try:
                ...
                logger.error(f"Error during sub-grouping for Security Name '{sec_name}' in {input_path}: {e}. Skipping this security.", exc_info=True)
                continue
        else:
...
def process_csv_file(input_path, output_path, date_columns, dates_file_path):
    ...
    try:
        ...
        df = replace_headers_with_dates(
            df, required_cols, candidate_start_index, candidate_cols, date_columns, dates_file_path, logger, input_path
        )
