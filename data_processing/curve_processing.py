...
def load_curve_data(data_folder_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        # Ensure Date column is datetime with improved error handling
        original_date_count = len(df)
        try:
            # First try the expected ISO format
            df["Date"] = pd.to_datetime(df["Date"], format="%Y-%m-%dT%H:%M:%S", errors="coerce")
            
            # If too many failures, try flexible parsing
            failed_conversions = df["Date"].isna().sum()
            if failed_conversions > 0:
                logger.warning(f"ISO format parsing failed for {failed_conversions} dates, trying flexible parsing")
                # Try flexible parsing for failed dates only
                failed_mask = df["Date"].isna()
                if failed_mask.any():
                    original_values = df.loc[failed_mask, "Date"]
                    # Reset the failed dates to their original values for reprocessing
                    df.loc[failed_mask, "Date"] = original_values
                    df.loc[failed_mask, "Date"] = pd.to_datetime(df.loc[failed_mask, "Date"], errors="coerce")
                    
        except Exception as e:
            logger.error(f"Error converting Date column in {file_path}: {e}")
            # Try to salvage what we can with flexible parsing
        ...
        if dropped_dates > 0:
            ...
            if dropped_dates > original_date_count * 0.1:  # More than 10% failed
                logger.error(
                    f"High failure rate for date parsing in {file_path}: {dropped_dates}/{original_date_count} rows dropped"
                )
