...
def run_data_consistency_audit(data_folder: str) -> Dict[str, Any]:
    ...
    report = {
        ...
        "skipped_files": [],
        "recommendations": [],
        "audit_error": None,  # Field to store top-level errors
    }
    ...
    try:  # Add top-level try block
        ...
        all_prefixes = ["ts_", "sp_ts_", "sec_", "att_factors_"]
        already_scanned = set()
                ...
                try:
                        ...
                        if date_col:
                            ...
                            if not dates.empty:
                                ...
                                file_info["issues"].append(
                                    f"Could not parse any valid dates in column '{date_col}'"
                                )
                                file_info["date_range"] = None
                        else:
                            file_info["issues"].append(
                                "No date column found (expected e.g. 'Position Date' or 'Date')"
                            )
                            file_info["date_range"] = None
                    # --- For sec_*.csv: date columns are columns (wide format) ---
                    elif prefix == "sec_":
                        file_info["date_columns"] = [
                            col for col in df.columns if _is_date_like(col)
                        ]
                        if file_info["date_columns"]:
                            ...
                            file_info["date_range"] = None
                    # --- For att_factors_*.csv: determine format automatically ---
                    elif prefix == "att_factors_":
                        # Try wide first (less common but possible)
                        file_info["date_columns"] = [
                            col for col in df.columns if _is_date_like(col)
                        ]
                        if file_info["date_columns"]:
                    ...
                    for idx, col in enumerate(file_info["columns"]):
                        if not str(col).strip():
                            file_info["issues"].append(
                                f"Blank column header at position {idx+1}"
                            )
                            report["recommendations"].append(
                ...
                if drange:
                    ...
                    if prefix == "sec_":
                        report["structure_issues"].append(
                            {"file": fname, "issue": "No date columns detected"}
                        )
        ...
        report["key_files"] = {
            ...
            "all_match": len(set(key_date_ranges.values())) == 1
            and len(key_date_ranges) > 0,
            ...
            for fname in files:
                ...
                try:
                    ...
                    for date in date_cols:
                        funds_with_data = set(
                            df.loc[~df[date].isna(), fund_col].astype(str)
                        )
                        missing_funds = all_funds - funds_with_data
                        if missing_funds:
                            missing.append(
                                {"date": date, "missing_funds": list(missing_funds)}
                            )
        ...
        report["summary"].append(
            {
                "check": "ts_*.csv date ranges match",
                "result": report["ts_files"]["all_match"],
            }
        )
        report["summary"].append(
            {
                "check": "sp_ts_*.csv date ranges match",
                "result": report["sp_ts_files"]["all_match"],
            }
        )
        report["summary"].append(
            {
                "check": "sec_*.csv date ranges match",
                "result": report["sec_files"]["all_match"],
            }
        )
        # Add check for att_factors_ date ranges if needed
        att_factors_files = find_files("att_factors_")
        att_factors_ranges = {}
        for fname in att_factors_files:
             info = report["file_details"].get(fname, {})
             drange = info.get("date_range", None)
             if drange:
                 att_factors_ranges[fname] = drange
        att_factors_all_match = len(set(att_factors_ranges.values())) == 1 and len(att_factors_ranges) > 0
        report["att_factors_files"] = {
            "date_ranges": att_factors_ranges,
            "all_match": att_factors_all_match,
            "unique_ranges": list(set(att_factors_ranges.values()))
        }
        report["summary"].append(
            {
                "check": "att_factors_*.csv date ranges match",
                "result": att_factors_all_match,
            }
        )
        for key in ["ts_files", "sp_ts_files"]:
            ...
            report["summary"].append(
                {"check": f"{key} fund presence", "result": not any_issues}
            )
        report["summary"].append(
            {
                "check": "File structure issues",
                "result": len(report["structure_issues"]) == 0,
            }
        )
        ...
        logger.error(f"CRITICAL ERROR during data audit: {e}", exc_info=True)
        report["audit_error"] = f"A critical error occurred during the audit: {e}"
        # Optionally add a generic recommendation if the audit failed critically
        report["recommendations"].append("Audit failed critically. Check server logs for details.")
