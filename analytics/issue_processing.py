...
REQUIRED_ISSUE_COLUMNS = [
    ...
    "ClosedBy",
    "ResolutionComment",
    "Comments",  # JSON string list of comment dicts
]
                ...
                for col in REQUIRED_ISSUE_COLUMNS:
                    if col not in existing_columns:
                        ...
                        if "Date" in col:
                            ...
                            df[col] = "[]"  # Initialise with empty JSON list
                        else:
        ...
        new_issue = pd.DataFrame(
            {
                ...
                "Comments": ["[]"], # Initialize Comments column
            }
        )
...
def _serialize_comments(comments: List[Dict[str, Any]]) -> str:
    import json
    return json.dumps(comments, ensure_ascii=False)


def _deserialize_comments(text: str) -> List[Dict[str, Any]]:
    import json
    try:
...
def add_comment_to_issue(issue_id: str, user: str, comment_text: str, data_folder_path: str) -> bool:
    """Append a timestamped comment to an issue."""
    try:
        df = load_issues(data_folder_path)
        df["IssueID"] = df["IssueID"].astype(str)
        idxs = df[df["IssueID"] == str(issue_id)].index
        if idxs.empty:
            logger.warning(f"Issue {issue_id} not found for commenting")
            return False
        idx = idxs[0]
        comments_serialized = df.loc[idx, "Comments"] if "Comments" in df.columns else "[]"
        comments_list = _deserialize_comments(comments_serialized)
        comments_list.append({
            "timestamp": datetime.now().isoformat(timespec="seconds"),
            "user": user,
            "comment": comment_text,
        })
        df.loc[idx, "Comments"] = _serialize_comments(comments_list)
        _save_issues(df, data_folder_path)
        return True
    except Exception as e:
...
def get_issue_by_id(issue_id: str, data_folder_path: str) -> Optional[pd.Series]:
    """Return issue row as Series or None."""
    df = load_issues(data_folder_path)
    if df.empty:
        return None
    row = df[df["IssueID"].astype(str) == str(issue_id)]
    if row.empty:
        return None
    return row.iloc[0]
