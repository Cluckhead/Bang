            ...
            try:
                ...
                for col in REQUIRED_ISSUE_COLUMNS:
                    if col not in existing_columns:
                        logger.info(f"Adding missing column '{col}' to issues DataFrame.")
                        if 'Date' in col:
                ...
                logger.warning(f"Issues file at {issues_file} is empty. Returning empty DataFrame.")
                return pd.DataFrame(columns=REQUIRED_ISSUE_COLUMNS)
            except pd.errors.ParserError as e:
                logger.error(f"Error parsing issues file {issues_file}: {e}", exc_info=True)
                return pd.DataFrame(columns=REQUIRED_ISSUE_COLUMNS)
            except Exception as e:
                logger.error(f"Error loading issues file: {e}", exc_info=True)
                return pd.DataFrame(columns=REQUIRED_ISSUE_COLUMNS)
        else:
            logger.info(f"Issues file not found at {issues_file}. Returning empty DataFrame.")
            return pd.DataFrame(columns=REQUIRED_ISSUE_COLUMNS)
    except Exception as e:
...
def _save_issues(df: pd.DataFrame, data_folder_path: str) -> None:
    ...
    try:
        ...
        logger.error(f"Permission denied when saving issues file: {issues_file}", exc_info=True)
    except OSError as e:
        logger.error(f"OS error when saving issues file: {issues_file}: {e}", exc_info=True)
    except Exception as e:
...
def close_issue(issue_id: str, closed_by: str, resolution_comment: str, data_folder_path: Optional[str]) -> bool:
        ...
        if os.path.exists(fund_list_file):
            try:
                ...
                if fund_code_col:
                    ...
                    logger.warning(f"Could not find a suitable fund code column ({', '.join(potential_cols)}) in {fund_list_file}")
                    return [] # Return empty if column not found
            except FileNotFoundError:
                logger.error(f"Fund list file not found at {fund_list_file}.")
                return []
            except pd.errors.EmptyDataError:
                logger.warning(f"Fund list file at {fund_list_file} is empty. Returning empty list.")
                return []
            except pd.errors.ParserError as e:
                logger.error(f"Error parsing fund list file {fund_list_file}: {e}", exc_info=True)
                return []
            except Exception as e:
                logger.error(f"Error loading fund list: {e}", exc_info=True)
                return []
        else:
            logger.info(f"Fund list file not found at {fund_list_file}. Returning empty list.")
            return []
    except Exception as e:
...
def initialize_issue_file(data_folder_path: str) -> None:
    ...
    try:
        ...
        logger.error(f"Error initializing issues file at {issues_file}: {e}", exc_info=True)
