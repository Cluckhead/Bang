...
def mock_staleness_logic(mocker):
    ...
    mock_exists = mocker.patch('app.views.staleness_views.os.path.exists', return_value=True)
    mock_load_data = mocker.patch('app.views.staleness_views.load_and_process_data') # Assuming this is used indirectly
    mock_get_summary = mocker.patch('app.views.staleness_views.get_staleness_summary', return_value=SAMPLE_SUMMARY.copy())
    mock_get_details = mocker.patch('app.views.staleness_views.get_stale_securities_details', return_value=SAMPLE_DETAILS.copy())
    mock_plot = mocker.patch('app.views.staleness_views.create_staleness_plot', return_value=SAMPLE_PLOT)
...
def test_staleness_page_no_data(client, mock_staleness_logic):
    ...
    mock_get_summary.side_effect = FileNotFoundError("Mock file not found for staleness")

    response = client.get(url_for('staleness_views.staleness_page'), follow_redirects=True)

    assert response.status_code == 200 # Handles gracefully
    messages = get_flashed_messages(with_categories=True)
    if messages:
        assert any('Error loading staleness data' in msg[1] for msg in messages) or \
               any('missing' in msg[1] for msg in messages)
    else:
        assert b'Error loading staleness data' in response.data or b'data file is missing' in response.data
