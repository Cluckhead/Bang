...
def mock_attribution_logic(mocker):
    ...
    mock_exists = mocker.patch(
        "app.views.attribution_views.os.path.exists", return_value=True
    )
    mock_load_data = mocker.patch(
        "app.views.attribution_views.load_and_process_data",
    ...
    mock_calc_residual = mocker.patch(
        "app.views.attribution_views.calc_residual",
        return_value=SAMPLE_RESIDUAL_STATS.copy(),
    )
    mock_norm = mocker.patch(
        "app.views.attribution_views.norm", return_value=SAMPLE_DATA.copy() * 0.1
...
def test_attribution_page_success(client, mock_attribution_logic):
    ...
    mock_exists, mock_load_data, mock_calc_metrics, mock_calc_residual, mock_norm = (
        mock_attribution_logic
    )
...
def test_attribution_page_no_data(client, mock_attribution_logic):
    ...
    response = client.get(
        url_for("attribution_views.attribution_page"), follow_redirects=True
    )
    ...
    if messages:
        assert any(
            "Error loading attribution data" in msg[1] for msg in messages
        ) or any("file not found" in msg[1] for msg in messages)
    else:
        assert (
            b"Error loading attribution data" in response.data
...
def test_attribution_page_processing_error(client, mock_attribution_logic):
    ...
    mock_calc_residual.side_effect = ValueError(
        "Mock processing error in residual calc"
    )
