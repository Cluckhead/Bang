...
def setup_data_folder(client, tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(
        client.application.config, "EXCLUSIONS_FILE", str(exclusions_file_path)
    )
    ...
    def mock_exists(path):
        try:
            ...
            if path_obj == exclusions_file_resolved or path_obj.is_relative_to(
                data_folder_resolved
            ):
...
def mock_exclusion_logic(mocker, setup_data_folder):
    ...
    mock_load = mocker.patch(
        "views.exclusion_views.load_exclusions",
        return_value=SAMPLE_EXCLUSIONS_DF.copy(),
    )
...
def test_exclusions_page_load_error(client, mock_exclusion_logic, setup_data_folder):
    ...
    response = client.get(
        url_for("exclusion_bp.exclusions_page"), follow_redirects=True
    )  # Use blueprint name
...
def test_add_exclusion_success(client, mock_exclusion_logic):
    ...
    exclusion_data = {"isin": "DE789", "reason": "Test Reason"}

    response = client.post(
        url_for("exclusion_bp.add_exclusion"),
        data=exclusion_data,
    ...
    messages = get_flashed_messages(with_categories=True)
    assert any(
        f'Exclusion for {exclusion_data["isin"]} added.' in msg[1] for msg in messages
    )
    ...
    mock_open_context.assert_called_once_with(
        str(exclusions_file_path), "a", newline=""
    )
...
def test_add_exclusion_missing_data(client, mock_exclusion_logic):
    ...
    exclusion_data = {"isin": "DE789"}  # Missing reason
