...
class TestApiViews:
    ...
    def test_run_api_calls_success(self, client, mocker):
        ...
        mocker.patch("os.path.exists", return_value=True)
        query_map_df = pd.DataFrame([{"QueryID": "Q1", "FileName": "ts_test.csv"}])
        mocker.patch("pandas.read_csv", return_value=query_map_df)
        mocker.patch(
            "views.api_routes_call._simulate_and_print_tqs_call", return_value=5
        )
        data = {
            "date_mode": "quick",
            "write_mode": "expand",
        ...
        response = client.post(
            "/run_api_calls", data=json.dumps(data), content_type="application/json"
        )
        assert response.status_code == 200
    ...
    def test_rerun_api_call_success(self, client, mocker):
        ...
        mocker.patch("pandas.read_csv", return_value=query_map_df)
        mocker.patch(
            "views.api_routes_call._simulate_and_print_tqs_call", return_value=3
        )
        data = {
            "query_id": "Q1",
            "days_back": 3,
        ...
        response = client.post(
            "/rerun-api-call", data=json.dumps(data), content_type="application/json"
        )
        assert response.status_code == 200
        resp_json = response.get_json()
    ...
    def test_add_schedule_success(self, client, mocker):
        ...
        response = client.post(
            "/schedules", data=json.dumps(data), content_type="application/json"
        )
