    ...
    df = pd.DataFrame(
        {
            ...
            "Code": [
                f"Fund{metric_name}",
                f"Fund{metric_name}",
            ],  # Or use Code directly if needed
            "Value": [100, 101],
...
@patch(
    "views.main_views.os.path.exists"
)  # Keep this mock if internal logic uses it, but prefer config
...
def test_dashboard_success(
    ...
    client,
    setup_data_folder,
):  # Use client from conftest, add setup_data_folder
    ...
    idx_a = pd.MultiIndex.from_tuples(
        [
            (pd.Timestamp("2024-01-01"), f"FundMetricA"),
            (pd.Timestamp("2024-01-02"), f"FundMetricA"),
        ],
        names=["Date", "Code"],
    )
    primary_df_a = pd.DataFrame(
        {"Value": [100, 101], "Benchmark": [f"BenchMetricA", f"BenchMetricA"]},
        index=idx_a,
    )
    idx_b = pd.MultiIndex.from_tuples(
        [
            (pd.Timestamp("2024-01-01"), f"FundMetricB"),
            (pd.Timestamp("2024-01-02"), f"FundMetricB"),
        ],
        names=["Date", "Code"],
    )
    primary_df_b = pd.DataFrame(
        {"Value": [100, 101], "Benchmark": [f"BenchMetricB", f"BenchMetricB"]},
        index=idx_b,
    )
    ...
    metrics_a = pd.DataFrame(
        {"FundMetricA Change Z-Score": [1.0], "Benchmark Change Z-Score": [0.5]}
    )
    metrics_b = pd.DataFrame(
        {"FundMetricB Change Z-Score": [-1.0], "Benchmark Change Z-Score": [-0.5]}
    )
    ...
    assert (
        b"FundMetricA - MetricA" in response.data
    )  # Example, adjust if template changes
    assert (
        b"Benchmark - MetricB" in response.data
    )  # Example, adjust if template changes
...
@patch("views.main_views.load_and_process_data")
@patch("views.main_views.calculate_latest_metrics")
def test_dashboard_no_ts_files(
    mock_calc_metrics,
    mock_load_data,
    mock_exists,
    mock_listdir,
...
def test_dashboard_calc_error(
    ...
    mock_calc_metrics.side_effect = ValueError(
        "Calculation failed"
    )  # Simulate error during calculation

    response = client.get("/")
    assert response.status_code == 200
    assert b"Dashboard" in response.data
    assert (
        b"Warning: Could not calculate latest_metrics for ts_MetricA.csv"
        in response.data
    )
    assert b"No Change Z-scores could be extracted" in response.data  # Should be empty
