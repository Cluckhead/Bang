...
def setup_data_folder(client, tmp_path, monkeypatch):
    ...
    def mock_exists(path):
        try:
            ...
            if path_obj == issues_file_path.resolve() or path_obj.is_relative_to(data_folder.resolve()):
    ...
    with patch('views.issue_views.os.path.exists', side_effect=mock_exists), \
         patch('issue_processing.os.path.exists', side_effect=mock_exists): # Patch in processing module too
...
def mock_issue_logic(mocker, setup_data_folder):
    ...
    mock_load_issues = mocker.patch('issue_processing.load_issues', return_value=SAMPLE_ISSUES_DF.copy())
    mock_add_issue = mocker.patch('issue_processing.add_issue', return_value='new_issue_id')
    mock_close_issue = mocker.patch('issue_processing.close_issue', return_value=True)
    mock_load_fund_list = mocker.patch('issue_processing.load_fund_list', return_value=['FUND1', 'FUND2'])

    # Mock other functions called by the view
    mock_load_users = mocker.patch('views.issue_views.load_users', return_value=['User1', 'User2'])

    return mock_load_issues, mock_add_issue, mock_close_issue, mock_load_fund_list, mock_load_users, data_folder_path
...
def test_add_new_issue_success(client, mock_issue_logic, mocker):
    ...
    response = client.post(url_for('issue_bp.add_issue'), data=issue_data, follow_redirects=True) # Use url_for
...
def test_add_new_issue_missing_data(client, mock_issue_logic):
    ...
    issue_data = {
        'raised_by': 'User1' # Missing required fields
    }
    ...
    messages = get_flashed_messages(with_categories=True)
    assert any('Missing required fields' in msg[1] for msg in messages) # Check for specific error flash
...
def test_close_existing_issue_success(client, mock_issue_logic):
    ...
    response = client.post(url_for('issue_bp.close_issue', issue_id=issue_id_to_close), data=close_data, follow_redirects=True) # Use url_for
