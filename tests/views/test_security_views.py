...
def create_dummy_exclusions(data_folder_path: Path) -> Path:
    ...
    exclusions_path = data_folder_path / 'Exclusions.csv' # Match expected filename case?
...
# Update helper to use pathlib
def create_dummy_security_file(data_folder_path: Path, filename: str="sec_Spread.csv") -> Path:
    ...
    df = pd.DataFrame({
        'ISIN': ['SEC001', 'SEC002', 'EXCLUDED01', 'SEC001', 'SEC002', 'EXCLUDED01'],
    ...
    df_pivoted = df.pivot_table(index=['ISIN', 'StaticCol'], columns='Date', values='Value').reset_index()
    df_pivoted.columns = ['ISIN', 'StaticCol'] + [d.strftime('%Y-%m-%d') for d in df_pivoted.columns[2:]]
...
def setup_data_folder(client, tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(client.application.config, 'EXCLUSIONS_FILE', str(exclusions_file))
    ...
    with patch('views.security_views.os.path.exists', side_effect=mock_exists), \
         patch('views.exclusion_views.os.path.exists', side_effect=mock_exists): # Also patch for exclusion view if needed
        yield data_folder
...
def test_securities_page_success(mock_calc_metrics, mock_load_data, client, setup_data_folder):
    ...
    idx = pd.MultiIndex.from_tuples([
        (pd.Timestamp('2024-01-01'), 'SEC001'), (pd.Timestamp('2024-01-01'), 'SEC002'), (pd.Timestamp('2024-01-01'), 'EXCLUDED01'),
        (pd.Timestamp('2024-01-02'), 'SEC001'), (pd.Timestamp('2024-01-02'), 'SEC002'), (pd.Timestamp('2024-01-02'), 'EXCLUDED01')
    ], names=['Date', 'ISIN'])
    ...
    response = client.get(url_for('security_bp.securities_page', security_type='Spread')) # Use blueprint name
...
@patch('views.security_views.load_and_process_security_data')
@patch('views.security_views.calculate_security_latest_metrics')
def test_securities_page_no_file(mock_calc_metrics, mock_load_data, client, setup_data_folder):
...
@patch('views.security_views.load_and_process_security_data')
@patch('views.security_views.calculate_security_latest_metrics')
def test_securities_page_empty_data(mock_calc_metrics, mock_load_data, client, setup_data_folder):
    data_folder = setup_data_folder
    ...
    mock_load_data.return_value = (pd.DataFrame(columns=['Date', 'ISIN']).set_index(['Date', 'ISIN']), [])

    response = client.get(url_for('security_bp.securities_page', security_type='Yield')) # Use blueprint name
    assert response.status_code == 200
    assert b'No security data found for Yield' in response.data or b'No data available' in response.data
    mock_calc_metrics.assert_not_called()


@patch('views.security_views.load_and_process_security_data')
@patch('views.security_views.calculate_security_latest_metrics')
def test_securities_page_empty_after_filtering(mock_calc_metrics, mock_load_data, client, setup_data_folder):
    data_folder = setup_data_folder
    ...
    idx = pd.MultiIndex.from_tuples([
        (pd.Timestamp('2024-01-01'), 'SEC001'), (pd.Timestamp('2024-01-01'), 'EXCLUDED01')
    ], names=['Date', 'ISIN'])
    ...
    metrics_df = pd.DataFrame({
        'ISIN': ['SEC001', 'EXCLUDED01'], 'StaticCol': ['A', 'Z'], 'Latest Value': [1.0, 99.0],
        'Change': [0.0, 0.0], 'Change Z-Score': [0.0, 0.0], 'Mean': [1.0, 99.0],
        'Max': [1.0, 99.0], 'Min': [1.0, 99.0], '_abs_z_score_': [0.0, 0.0]
    })
    mock_calc_metrics.return_value = metrics_df

    # Test with search term that matches nothing *after* exclusion
    response = client.get(url_for('security_bp.securities_page', security_type='Rate', search_term='SEC001')) # Use blueprint name
    assert response.status_code == 200
    ...
    response = client.get(url_for('security_bp.securities_page', security_type='Rate', search_term='ZZZ')) # Use blueprint name
    assert response.status_code == 200
...
@patch('views.security_views.load_and_process_security_data')
@patch('views.security_views.calculate_security_latest_metrics')
def test_securities_page_pagination(mock_calc_metrics, mock_load_data, client, setup_data_folder):
    data_folder = setup_data_folder
    create_dummy_exclusions(data_folder) # Excludes EXCLUDED01 if generated
    sec_file_path = create_dummy_security_file(data_folder, "sec_Volume.csv") # Creates SEC001, SEC002, EXCLUDED01

    # Create more data for pagination (let's generate 55 non-excluded)
    num_items = 55
    isins = [f'PAGE{i:03d}' for i in range(num_items)] # + SEC001, SEC002 = 57 total non-excluded
    ...
    for isin in isins:
        for i, date in enumerate(dates):
            page_data.append({'ISIN': isin, 'Date': date, 'Value': 100 + i, 'StaticCol': 'X'})
    ...
    for i, isin in enumerate(all_isins):
        ...
        metrics_list.append({
            ...
            'Latest Value': latest_val, 'Change': change, 'Change Z-Score': 0.5, # Dummy Z
            'Mean': mean, 'Max': latest_val, 'Min': mean - 0.5, '_abs_z_score_': 0.5 # Dummy sort
        })
    ...
    # Test page 1
    response = client.get(url_for('security_bp.securities_page', security_type='Volume', page=1)) # Use blueprint name
    assert response.status_code == 200
    ...
    # Test page 2
    response = client.get(url_for('security_bp.securities_page', security_type='Volume', page=2)) # Use blueprint name
    assert response.status_code == 200
...
@patch('views.security_views.load_and_process_security_data')
@patch('views.security_views.calculate_security_latest_metrics') # Calc may not be needed if only loading
def test_security_details_success(mock_calc_metrics, mock_load_data, client, setup_data_folder):
    data_folder = setup_data_folder
    ...
    idx = pd.MultiIndex.from_tuples([
        (pd.Timestamp('2024-01-01'), 'SEC001'), (pd.Timestamp('2024-01-02'), 'SEC001')
    ], names=['Date', 'ISIN'])
    ...
    response = client.get(url_for('security_bp.security_details', security_type=security_type, security_id=security_id)) # Use blueprint name
    assert response.status_code == 200
