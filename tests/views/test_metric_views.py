...
def create_dummy_metric_file(
    data_folder_path: Path, metric_name: str, is_secondary: bool = False
) -> Path:
...
def test_metric_page_success(
    mock_calc_metrics, mock_load_data, client, setup_data_folder
):  # Use client, setup_data_folder
    metric_name = "Yield"
    data_folder = setup_data_folder  # Use path from fixture
    primary_file = create_dummy_metric_file(
        data_folder, metric_name, is_secondary=False
    )
    secondary_file = create_dummy_metric_file(
        data_folder, metric_name, is_secondary=True
    )

    # Mock data loading (needs to be called twice: primary then secondary)
    idx_p = pd.MultiIndex.from_tuples(
        [
            (pd.Timestamp("2024-01-01"), f"Fund{metric_name}"),
            (pd.Timestamp("2024-01-02"), f"Fund{metric_name}"),
        ],
        names=["Date", "Code"],
    )
    primary_data = pd.DataFrame(
        {"Value": [100, 101], "Benchmark": [f"Bench{metric_name}"] * 2}, index=idx_p
    )
    idx_s = pd.MultiIndex.from_tuples(
        [
            (pd.Timestamp("2024-01-01"), f"Sec{metric_name}"),
            (pd.Timestamp("2024-01-02"), f"Sec{metric_name}"),
        ],
        names=["Date", "Code"],
    )
    secondary_data = pd.DataFrame(
        {"Value": [50, 51], "Benchmark": [f"SecBench{metric_name}"] * 2}, index=idx_s
    )
    ...
    response = client.get(
        url_for("metric_bp.metric_page", metric_name=metric_name)
    )  # Ensure correct blueprint name
...
@patch("views.metric_views.load_and_process_data")
@patch("views.metric_views.calculate_latest_metrics")
def test_metric_page_missing_file(
    mock_calc_metrics, mock_load_data, client, setup_data_folder
):  # Use client, setup_data_folder
...
@patch("views.metric_views.load_and_process_data")
@patch("views.metric_views.calculate_latest_metrics")
def test_metric_page_empty_data(
    mock_calc_metrics, mock_load_data, client, setup_data_folder
):  # Use client, setup_data_folder
    metric_name = "Yield"
...
@patch("views.metric_views.load_and_process_data")
@patch("views.metric_views.calculate_latest_metrics")
def test_metric_page_secondary_missing(
    mock_calc_metrics, mock_load_data, client, setup_data_folder
):  # Use client, setup_data_folder
    metric_name = "Yield"
    data_folder = setup_data_folder
    ...
    assert (
        b"Secondary data file (sp_ts_Yield.csv) not found or failed to load."
        in response.data
    )
...
@patch("views.metric_views.load_and_process_data")
@patch("views.metric_views.calculate_latest_metrics")
def test_metric_page_metric_calc_error(
    mock_calc_metrics, mock_load_data, client, setup_data_folder
):  # Use client, setup_data_folder
    metric_name = "Yield"
    data_folder = setup_data_folder
