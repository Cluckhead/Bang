...
SAMPLE_CURVE_DATA = pd.DataFrame(
    {"1m": [0.01, 0.011], "3m": [0.015, 0.016], "1y": [0.02, 0.021]},
...
def setup_data_folder(client, tmp_path, monkeypatch):
    ...
    def mock_exists(path):
        try:
            ...
            if path_obj == curve_file_path.resolve() or path_obj.is_relative_to(
                data_folder.resolve()
            ):
    ...
    with patch("views.curve_views.os.path.exists", side_effect=mock_exists), patch(
        "curve_processing.os.path.exists", side_effect=mock_exists
...
def mock_curve_logic(mocker, setup_data_folder):
    ...
    mock_load_curve = mocker.patch(
        "curve_processing.load_curve_data", return_value=SAMPLE_CURVE_DATA.copy()
    )
    mock_get_latest = mocker.patch(
        "curve_processing.get_latest_curve_date", return_value=LATEST_CURVE_DATE
    )
    mock_check_incons = mocker.patch(
        "curve_processing.check_curve_inconsistencies",
        return_value=SAMPLE_INCONSISTENCIES.copy(),
    )
...
def test_curve_page_no_data_file(client, mock_curve_logic, setup_data_folder):
    ...
    response = client.get(
        url_for("curve_bp.curve_page"), follow_redirects=True
    )  # Use blueprint name
...
def test_curve_page_processing_error(client, mock_curve_logic):
    ...
    mock_load_curve.assert_called_once_with(str(data_folder))
    mock_get_latest.assert_called_once_with(SAMPLE_CURVE_DATA)
    mock_check_incons.assert_called_once_with(
        SAMPLE_CURVE_DATA
    )  # Called but raised error
