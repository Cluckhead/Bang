...
def mock_weight_logic(mocker):
    ...
    mock_exists = mocker.patch('app.views.weight_views.os.path.exists', return_value=True)
...
def test_weights_page_no_data_file(client, mock_weight_logic):
    ...
    if messages:
        ...
        assert b'Weight file not found' in response.data or b'No weight data available' in response.data
