...
def test_calculate_column_stats_basic():
    ...
    expected_z = (1 - mean_change) / std_change if not np.isnan(std_change) and std_change != 0 else 0.0
...
def test_calculate_column_stats_nan():
    ...
    assert np.isnan(result[f"{col_name} Change Z-Score"]) or isinstance(result[f"{col_name} Change Z-Score"], float)
...
def test_calculate_latest_metrics_edge_cases():
    ...
    for col in result.columns:
        if "Change" in col:
            assert np.isnan(result.iloc[0][col]) or isinstance(result.iloc[0][col], float) 