...
def test_load_and_process_security_data_valid(tmp_path):
    ...
    result, static_cols = security_processing.load_and_process_security_data('sec_test.csv', str(tmp_path))
    assert not result.empty
    assert 'ISIN' in result.columns or 'ISIN' in result.index.names
    assert 'date' in [c.lower() for c in result.columns] or 'date' in [c.lower() for c in result.index.names]
...
def test_calculate_security_latest_metrics():
    ...
    result = security_processing.calculate_security_latest_metrics(df, static_cols=['StaticCol'])
    assert not result.empty
