...
def test_replace_headers_with_dates():
    ...
    result = process_data.replace_headers_with_dates(
        df.copy(), required_cols, candidate_start_index, candidate_cols, date_columns, dates_file_path, logger, input_path
    )
...
def test_aggregate_data():
    ...
    assert any('[' in str(f) for f in result['Funds'])  # Funds aggregated as list-like string 