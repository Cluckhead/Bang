...
def test_process_single_file_valid(tmp_path):
    ...
    df = pd.DataFrame({'Code': ['A', 'B'], 'Value': [1, 2], 'Date': ['2024-01-01', '2024-01-02'], 'Benchmark': [10, 20]})
...
def test_load_and_process_data_valid(tmp_path):
    ...
    df.to_csv(file_path, index=False)
    result = data_loader.load_and_process_data('primary.csv', data_folder_path=str(tmp_path))
