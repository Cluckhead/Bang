...
def detect_stale_data(file_path, placeholder_values=[100], consecutive_threshold=3, quiet=False):
    """
    ...
    try:
        ...
        if df.shape[1] < 7 and not quiet:  # Need at least ID, name, metadata columns + some data columns
            print(f"Warning: File format may be incorrect. Found only {df.shape[1]} columns.")
            return pd.DataFrame()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Detect stale data in financial CSV files")
    parser.add_argument("file_path", help="Path to the CSV file to analyze")
    parser.add_argument("--placeholder", type=int, nargs="+", default=[100], 
    ...
    parser.add_argument("--compact", action="store_true", help="Display results in compact format")
    ...
    if not results.empty:
        ...
        if not args.quiet:
            if args.compact:
                print(f"Results: {total_count} securities, {stale_count} stale ({stale_count/total_count*100:.1f}%)")
            else:
                print(f"\nAnalysis Results:")
                print(f"Total securities: {total_count}")
                print(f"Stale securities: {stale_count} ({stale_count/total_count*100:.1f}%)")
            ...
            if stale_count > 0 and not args.compact:
                ...
                for idx, row in stale_df.iterrows():
                    print(f"- {row['security_id']} ({row['security_name']}): Stale from {row['stale_start_date']}, {row['stale_consecutive_values']} consecutive values")
