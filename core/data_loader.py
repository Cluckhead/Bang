...
def _convert_value_columns(df: pd.DataFrame, value_cols: list) -> list:
    ...
    for col in value_cols:
        if col in df.columns:
            # Store pre-conversion info for better error reporting
            ...
            try:
                ...
                # Log conversion statistics for problematic columns
                post_conversion_count = df[col].notna().sum()
                if post_conversion_count < non_null_count:
                    conversion_failures = non_null_count - post_conversion_count
                    logger.info(
                        f"Column '{col}': {conversion_failures} values failed numeric conversion. "
                        f"Original non-null: {non_null_count}, After conversion: {post_conversion_count}"
                    )
                    
            except Exception as e:
                logger.error(f"Failed to convert column '{col}' to numeric: {e}")
                # Keep the column as-is rather than failing completely
                continue
...
def _process_single_file(
    ...
    try:
        ...
        # Improved handling for Fund Duration and other critical columns
        for col in original_fund_val_col_names:
            ...
            if nan_count > 0:
                ...
                sample_nan_rows = df[df[col].isna()].head(3)
                if not sample_nan_rows.empty:
                    logger.info(
                        f"Sample rows with NaN values in '{col}' from {filename_for_logging}:\n{sample_nan_rows.to_string()}"
                    )
                
                # Check if this is a duration-related file and provide more context
                if 'duration' in filename_for_logging.lower() and nan_count > 10:
                    logger.warning(
                        f"High number of NaN values ({nan_count}) in '{col}' from {filename_for_logging}. "
                        f"This may indicate data quality issues or missing fund information."
                    )
