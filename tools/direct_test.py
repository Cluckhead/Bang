...
def main():
    ...
    print("\nValue analysis:")
    print(f"{'Index':5s} | {'Date':10s} | {'Value':8s} | {'Is 100?':7s} | {'Consecutive':10s}")
    print("-" * 50)
    ...
    for i, (date, val) in enumerate(zip(date_columns, values)):
        ...
        print(f"{i:5d} | {date:10s} | {val!s:8s} | {'Yes' if is_placeholder else 'No':7s} | {consecutive_count:10d}")
        
        # Check for staleness threshold
        if consecutive_count == CONSECUTIVE_THRESHOLD:
            print(f"FOUND {CONSECUTIVE_THRESHOLD} CONSECUTIVE PLACEHOLDERS - THIS IS STALE DATA")
            print(f"Starting at index {stale_start_idx}, date: {date_columns[stale_start_idx]}")
