...
try:
    from staleness_processing import get_stale_securities_details, DEFAULT_PLACEHOLDER_VALUES
except ImportError:
    print("Error: Could not import staleness_processing module. Make sure it's in the current directory.")
    sys.exit(1)
...
def run_test(data_file, threshold_days=5):
    ...
    try:
        ...
        if stale_securities:
            ...
            print(f"{'ID':<15} {'Name':<20} {'Currency':<10} {'Last Update':<15} {'Days Stale':<10} {'Stale Type':<15}")
            print("-" * 90)
            ...
            for sec in stale_securities:
                ...
                print(f"{sec_id:<15} {sec_name:<20} {currency:<10} {last_update:<15} {days_stale:<10} {stale_type:<15}")
...
def main():
    parser = argparse.ArgumentParser(description="Test the staleness detection algorithm")
    ...
    parser.add_argument("--check-security", help="Check if a specific security ID is detected as stale")
