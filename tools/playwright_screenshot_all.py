...
from core.config import PLAYWRIGHT_EXAMPLE_DATA, PLAYWRIGHT_SELECTORS
...
# Accept page_name so screenshots are uniquely identified per page
def open_modal(page, modal_type, page_name):
    selector = None
    ...
    log(f"    Attempting modal: {modal_type} on page {page_name}")
    ...
    try:
        ...
        time.sleep(0.5)
        # Include page name in screenshot for uniqueness
        page.screenshot(
            path=SCREENSHOT_DIR / f"{page_name}_modal_{modal_type}.png", full_page=True
        )
...
# Accept page_name so screenshots are uniquely identified per page
def perform_ui_interaction(page, interaction_type, page_name):
    log(f"  Performing UI interaction: {interaction_type} on page {page_name}")
    if interaction_type == "toggle_sp_comparison":
        ...
        if toggle:
            try:
                toggle.click()
                log(f"    Clicked S&P toggle on {page_name}.")
                time.sleep(1)  # Wait for UI update
                screenshot_path = SCREENSHOT_DIR / f"{page_name}_{interaction_type}.png"
            ...
            try:
                ...
                if not page_had_errors:
                    for interaction in interactions:
                        perform_ui_interaction(page, interaction, name)
                elif interactions:
                    log(
