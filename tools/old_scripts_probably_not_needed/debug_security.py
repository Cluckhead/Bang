...
def _resolve_data_folder() -> str:
    try:
        ...
        if settings_path.exists():
            ...
            dfolder = (settings.get('app_config') or {}).get('data_folder') or "Data"
        else:
            dfolder = "Data"
        return dfolder if os.path.isabs(dfolder) else str(project_root / dfolder)
    except Exception:
        return str(Path(__file__).resolve().parents[1] / "Data")

DEBUG_FILE = os.path.join(_resolve_data_folder(), "sec_Spread.csv")
...
def debug_security():
    ...
    try:
        ...
        for i, (col, val) in enumerate(zip(date_columns, values)):
            ...
            print(
                f"{i:5d} | {col:10s} | {val!s:6s} | {'Yes' if is_100 else 'No'} {'← Start of ' + str(consecutive_100s) + ' consecutive 100s' if is_100 and consecutive_100s == 1 else ''}"
            )

            # Highlight potential staleness pattern
            if consecutive_100s == 3:
                print(
                    f"{'':5s} | {'':10s} | {'':6s} | ^ Found 3 consecutive 100s - should be detected as stale!"
                )
        ...
        if max_consecutive_100s >= 3:
            print(
                "This security SHOULD be detected as stale based on having 3+ consecutive 100s."
            )
            if stale_start_idx is not None:
                stale_start_date = date_columns[stale_start_idx]
                print(
                    f"Staleness begins at: {stale_start_date} (index {stale_start_idx})"
                )
        else:
            print(
                "This security does not have 3+ consecutive 100s, so it might not be detected as stale by that method."
            )
